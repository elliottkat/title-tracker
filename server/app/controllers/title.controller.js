...
exports.findOne = (req, res) => {
    ...
    Title.findById(id)
        .then((data) => {
            if (!data) res.status(404).send({ message: `Title ${id} was not found.` });
            else res.send(data);
            console.log('data:', data);
        })
        .catch((err) => {
            res.status(500).send({ message: `Error retrieving Title ${id}: ${err}` });
        });
};
...
exports.deleteAll = (req, res) => {
    Title.deleteMany({})
        ...
        .catch((err) => {
            res.status(500).send({
                message: err.message || 'Some error occurred while removing all Titles.',
            });
        });
};
